...
def extract_password_from_caption(caption):
    """Extract password from file caption"""
    if not caption:
        return None
    
    # Common patterns for passwords in captions
    patterns = [
        r'password[:\s]+([^\s\n]+)',
        r'pass[:\s]+([^\s\n]+)',
        r'pwd[:\s]+([^\s\n]+)',
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            print(f"🔐 Found password in caption: {password}")
            return password
    
    return None


def extract_archive_sync(archive_path, extract_dir, password=None):
    """Synchronous extraction for threading - supports multiple formats and passwords"""
    try:
        ...
        # ZIP files
            ...
            if password:
                with zipfile.ZipFile(archive_path, 'r') as archive:
                    archive.extractall(path=extract_dir, pwd=password.encode())
            else:
                with zipfile.ZipFile(archive_path, 'r') as archive:
                    archive.extractall(path=extract_dir)
...
async def download_and_extract_archives():
    ...
    try:
        ...
        for idx, (message, filename) in enumerate(files_to_process, 1):
            ...
            # Check for password in message caption
            archive_password = None
            if message.message:
                archive_password = extract_password_from_caption(message.message)
                if archive_password:
                    print(f"🔐 Password-protected archive detected")
                    await notify_admins(f"🔐 Password found for {filename}: {archive_password}")
            ...
            if is_excel_file:
                ...
                try:
                    ...
                    # Index into database
                    if config.USE_DATABASE_INDEX:
                        print(f"      🗄️ Indexing {csv_filename} into database...")
                        csv_content = '\n'.join(converted_lines)
                        database.index_file(csv_filename, csv_content, msg.id)
                        print(f"      ✅ Indexed {csv_filename}")
            ...
            success = await loop.run_in_executor(
                ...
                file_path, 
                extract_dir,
                archive_password  # Pass password to extraction function
            )
            ...
            if success:
                ...
                for file_to_upload in files_to_upload:
                    try:
                        ...
                        # Index into database
                        if config.USE_DATABASE_INDEX and not database.is_file_indexed(file_to_upload.name):
                            try:
                                if is_text_file(file_to_upload):
                                    with open(file_to_upload, 'r', encoding='utf-8', errors='ignore') as f:
                                        content = f.read()
                                    database.index_file(file_to_upload.name, content, msg.id)
                            except:
                                pass
...
async def search_data(query, search_type):
    """Search through indexed data or channel files"""
    results = []
    query_lower = query.lower()
    
    # Database index mode (for 10M+ files) - FASTEST
    if config.USE_DATABASE_INDEX:
        print(f"🗄️ Searching database index...")
        results = database.search_database(query_lower)
        print(f"✅ Found {len(results)} matches in database")
        return results
    
    # If full cache is enabled and loaded, use it (fast)
...
async def clear_cache_handler(event):
    ...
    buttons = [
        [Button.inline(button_text, b"toggle_process")],
        [Button.inline("🔄 Refresh Stats", b"refresh_stats"), Button.inline("🗄️ Get DB", b"download_db")],
        [Button.inline("📊 Channel Stats", b"channel_stats"), Button.inline("🗑️ Clear Cache", b"clear_cache")]
    ]
    
    await event.edit(text, buttons=buttons)


@bot.on(events.CallbackQuery(pattern=b"download_db"))
async def download_db_handler(event):
    """Send database file to admins"""
    if event.sender_id not in config.ADMIN_IDS:
        await event.answer("❌ Unauthorized!", alert=True)
        return
    
    await event.answer("📤 Sending database file...")
    
    try:
        # Check if database exists
        db_path = Path("breach_index.db")
        if not db_path.exists():
            await event.answer("❌ Database file not found!", alert=True)
            return
        ...
        for admin_id in config.ADMIN_IDS:
            try:
                await bot.send_file(
                    admin_id,
                    db_path,
                    caption=f"🗄️ **Breach Index Database**\n\n"
                            f"💾 Size: {db_size_mb:.2f} MB\n"
                            f"📊 Stats: {stats['total_files']} files indexed"
                )
            except Exception as e:
                print(f"Error sending DB to admin {admin_id}: {e}")
        
        await event.answer(f"✅ Database sent! ({db_size_mb:.2f} MB)", alert=True)
...
async def main():
    ...
    # Initialize database if using database index mode
    if config.USE_DATABASE_INDEX:
        print("🗄️ Initializing database index...")
        database.init_database()
        db_stats = database.get_stats()
        print(f"📊 Database stats: {db_stats['total_files']:,} files, {db_stats['total_lines']:,} lines indexed")
